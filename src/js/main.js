...
document.querySelectorAll('.tag-badge').forEach(function(tag) {
    tag.addEventListener('click', function() {
		...
		if(tag.querySelector('span').classList.contains('gradient-text')){
			unklik();
			unhide();
		}else{
			unklik();
			tag.querySelector('span').classList.add('gradient-text');
			const targetTag = tag.innerText.replace("# ", "");
			document.querySelectorAll('.project-card').forEach(function(project) {
				project.classList.add('hidden');
				project.querySelectorAll('.text-xs').forEach(function(span) {
					const tags = span.innerText;
					if(targetTag == "Database"){
						["JSON", "SQL"].forEach(function(q) {
							if(tags == q){
...
function unklik(){
	document.querySelectorAll('.tag-badge').forEach(function(tag) {
		tag.querySelector('span').classList.remove('gradient-text');
	});
}
